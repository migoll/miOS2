...
interface FileSystemStore {
  ...
  createEntity: (entity: Omit<FileSystemEntity, 'id' | 'createdAt' | 'updatedAt'>) => string;
...
export const useFileSystemStore = create<FileSystemStore>()(
  persist(
    (set, get) => ({
      entities: { root: createRootFolder() },
      rootFolderId: 'root',
      ...
      moveEntity: (id, newParentId) => {
        set((state) => {
          ...
          const newPath = newParent.path === '/' ? `/${entity.name}` : `${newParent.path}/${entity.name}`;
          newEntities[id] = {
            ...entity,
